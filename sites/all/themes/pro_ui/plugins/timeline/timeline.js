...
function Timeline(element, data, $, undefined) {
  ...
  this._createElement = function (element_data, position, index) {
    ...
    if (position === 'left') {
      ...
      element.addClass('timeline_element_left_' + index);
    } else if (position === 'right') {
      element.addClass('timeline_element_right');
      element.addClass('timeline_element_right_' + index);
    }
    ...
    if (element_data.title) {
      var title_html = '<div class="timeline_title_label clearfix">';
      if (element_data.flag) {
       var title_html = title_html + '<div class="country-flag">' +element_data.flag + '</div>';  //$('<div>').addClass('country-flag').html(element_data.flag).appendTo(element_box);      
    }
      var title_html = title_html + '<span class="timeline_title_user_photo">' + element_data.user_photo + '</span>';
      var title_html = title_html + '<div class="timeline_title_title">'      
      var title_html = title_html + '<div class="timeline_title_title_line_1"><span class="timeline_title_user_name">' + element_data.user_name + '</span>' + element_data.title + '</span></div>';
      var title_html = title_html + '<div class="timeline_title_title_line_2"><span class="timeline_title_date">' + element_data.date_formatted + ' at ' + element_data.time_formatted + '</span></div>';    
      var title_html = title_html + '</div>';//Close timeline_title_date span 
      var title_html = title_html + '</div>';//Close timeline_title_label span                 
      var title = $('<div>').addClass('timeline_title').html(title_html).appendTo(element_box);
    } else {
    ...
    if (element_data.logo) {
       $('<div>').addClass('project_logo').html(element_data.logo).appendTo(element_box);      
    }
    ...
    if (element_data.content) {
     ...
     if (element_data.images != '') {
      html += '<div class="img_container clearfix">';
      html += element_data.images;
      html += '</div>';
      $('<div>').html(html).appendTo(element_box);
    }        
    // content suffix DIV
    if (element_data.content_suffix) {
      $('<div>').addClass('content_suffix').html(element_data.content_suffix).appendTo(element_box);
    }
  ...
  this._render = function (timeline_data, is_append) {
    ...
    $(timeline_data).each(function (index, data) {
      ...
      if (SELF._options.columnMode === 'dual') {
        SELF._createElement(data, is_odd ? 'left' : 'right', is_odd ? left_index : right_index);
        left_index = (is_odd ? (left_index + 1) : left_index);
        right_index = (is_odd ? right_index : (right_index + 1));
    ...
    switch (parseInt(e.which, 10)) {
      ...
      case 37:   // LEFT
        if (SELF._lightbox.hasClass('loaded') && SELF._lightbox.find('.navigation span.prev').is(':visible')) {
          $('.navigation span.prev').trigger('click');
//          SELF._lightbox.find('.navigation span.prev').click();
      ...
      case 39:   // RIGHT
        if (SELF._lightbox.hasClass('loaded') && SELF._lightbox.find('.navigation span.next').is(':visible')) {
          $('.navigation span.next').trigger('click');
