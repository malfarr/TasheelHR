  ...
  (function ($, Drupal, window, document, undefined) {
    Drupal.behaviors.med_behavior = {
      attach: function (context, settings) {
        $('.household-member-activate-form').validate({
        ...
        $('.household-member-activate-form .form-submit').click(function () {
          for (var i in CKEDITOR.instances) {
            CKEDITOR.instances[i].updateElement();
            $.trim($('#' + i).val());
          }
          if ($(".household-member-activate-form").valid()) {
            $(".household-member-activate-form").submit();
          } else {
            return false;
          }
        });
